...
const WaterReadings = () => {
  ...
  const { data: readingsData, isLoading } = useQuery({
    queryKey: ['water-readings', filterTenantId, filterYear, filterMonth],
    queryFn: () => {
      const params = {};
      ...
      return waterReadingsService.getAll(params);
    },
  });
  ...
  const createMutation = useMutation({
    mutationFn: (values) => waterReadingsService.create(values),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, values }) => waterReadingsService.update(id, values),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => waterReadingsService.delete(id),
    onSuccess: () => {
  ...
  return (
    <div>
      <h1 style={{ marginBottom: 16 }}>Indexuri Apă</h1>
      ...
      <Modal
        title={editingRecord ? 'Editare Index Apă' : 'Adăugare Index Apă'}
        open={isModalOpen}
        onOk={handleOk}
