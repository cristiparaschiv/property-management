package PropertyManager::Routes::WaterReadings;
...
get '' => sub {
    ...
    my @rows = schema->resultset('WaterReading')->search($search, {
...
get '/:id' => sub {
    ...
    my $row = schema->resultset('WaterReading')->find(route_parameters->get('id'));
...
post '' => sub {
    ...
    try {
        ...
        my $prev = schema->resultset('WaterReading')->search(
        ...
        $created = schema->resultset('WaterReading')->create({
            tenant_id => $d->{tenant_id},
